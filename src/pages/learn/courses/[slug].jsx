...
export async function getStaticPaths() {
  ...
  return {
    paths: data.courseCollection.items.map((course) => ({
...
export async function getStaticProps(context) {
  const data = await getCourse(context.params.slug);

  return {
    props: { course: data.courseCollection.items[0] },
  };
}
...
export default function CoursePage({ course, renderH2Links }) {
  return (
    <Wrapper>
      <Head>
        <title>Code Shape - Courses - {course.title}</title>
            ...
            <Author className="mt-6 grid grid-flow-col auto-cols-max justify-center content-center">
              ...
              <div className="ml-3">
                <p className="text-sm font-medium text-indigo-600">
                  <a href={course.instructor.slug} className="hover:underline">
          ...
          <div className="mt-6 prose md:prose-xl dark:prose-dark dark:md:prose-xl-dark mx-auto">
            <ImageWrapper className="mt-8">
              <img
                ...
                alt={course.featuredImage.fileName}
